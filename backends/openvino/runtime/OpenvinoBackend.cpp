...
FuncPtr load_symbol(void* handle, const char* name) {
  ...
  if (!sym) {
    ET_LOG(
        Error,
        "OpenVINO: failed to resolve symbol '%s' (GetProcAddress error=%lu)",
        name,
...
#ifdef _WIN32
    void* handle = reinterpret_cast<void*>(LoadLibraryA(effective_path));
    if (!handle) {
      ET_LOG(
          Error,
          "OpenVINO runtime not found (LoadLibrary failed, error=%lu). "
          "Ensure 'openvino_c.dll' is on PATH or set OPENVINO_LIB_PATH to the "
          "full DLL path, or install with: "
